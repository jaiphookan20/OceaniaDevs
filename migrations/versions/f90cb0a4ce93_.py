...
def upgrade():
    ...
    op.alter_column('seekers', 'city',
                    existing_type=sa.String(length=255),
                    nullable=True)
    op.alter_column('seekers', 'state',
                    existing_type=sa.Enum('VIC', 'NSW', 'ACT', 'WA', 'QLD', 'NT', 'TAS', 'SA', name='state_enum'),
                    nullable=True)
    op.alter_column('seekers', 'country',
                    existing_type=sa.Enum('Australia', 'New Zealand', name='country_enum'),
                    nullable=True)
    op.alter_column('seekers', 'first_name',
                    existing_type=sa.String,
                    nullable=True)
    op.alter_column('seekers', 'last_name',
                    existing_type=sa.String,
                    nullable=True)
    #Recruiter Table Changes
    op.alter_column('recruiters', 'first_name',
                    existing_type=sa.String,
                    nullable=True)
    op.alter_column('recruiters', 'last_name',
                    existing_type=sa.String,
                    nullable=True)
    op.alter_column('recruiters', 'password',
                    existing_type=sa.String,
                    nullable=True)
