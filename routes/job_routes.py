...
cors = CORS(job_blueprint, resources={r"/*": {"origins": "http://localhost:3000"}})  # Update this line
...
company_logos = {
    ...
    'oceaniadevs': 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSQNq25SGwUGZGM72wsSTmoBtUK92v50s_sYQ&s',
...
#     return jsonify([{
#         'title': job.title,
#         'company': job.company_name,  # Assuming you have joined company name in the query
#         'location': f"{job.city}, {job.state}",
#         'salary': job.salary_range,
#         'date': job.created_at.strftime('%Y-%m-%d'),  # Adjust date format as needed
#         'logo': company_logos.get(job.company_name.lower(), ''),  # Use company logos dictionary
#         'new': job.created_at >= datetime.datetime.utcnow() - datetime.timedelta(days=7)  # Mark as new if created within the last week
#     } for job in jobs])
